...
```console
$ whoami
varun sharma — cs student, upstream contributor
$ cat ./focus
...
│ CLEAR     MERGE ACCESS  ·  9 ACCEPTED  ·  0 REVERTED     │
│ LAST SEEN 2026-08-23                                     │
└──────────────────────────────────────────────────────────┘
```

<samp>
I do not go wide. I pick one codebase and stay in it until the maintainers
stop reviewing my PRs line by line.
</samp>
...
Thirty-eight days inside `AOSSIE-Org/PictoPy`. Bar length is review depth — how
hard each change was fought over before it landed.

```text
┌─ TRACE  ·  38-day intrusion log ─────────────────────────┐
│ jul 16  ██  #1371  yolo model manager desync             │
│ jul 19  ██  #1384  docs / download alignment             │
│ jul 20  ██  #1387  navbar scroll regression              │
│ jul 24  ██  #1380  empty folder on early tagging         │
│ aug 04  ███ #1424  pfp uploads      44 comments          │
│ aug 04  ██  #1443  folder loading cleanup                │
│ aug 04  ██  #1447  semantic-search hang                  │
│ aug 17  ████ #1464 serial indexing → parallel            │
│ aug 23  ██  #1466  crop dialog fix                       │
├──────────────────────────────────────────────────────────┤
│ depth increases downward. entry was a docs fix.          │
└──────────────────────────────────────────────────────────┘
```
...
On-device image gallery. Detects objects, clusters faces, sends nothing anywhere.
I am a contributor with nine merges, most recently in the indexing path.
...
Realtime chat. Websockets rather than polling, presence rather than refresh.
Authored end to end.
